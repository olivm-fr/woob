...
class AmeliBrowser(TwoFactorBrowser):
    ...
    new_password_page = URL(
        r"/PortailAS/appmanager/PortailAS/assure\?.*as_modif_code_perso_ameli_apres_reinit_page", NewPasswordPage
    )
    redirect_page = URL(
        r"/PortailAS/appmanager/PortailAS/assure\?_nfpb=true&.*validationconnexioncompte.*", RedirectPage
    )
    cgu_page = URL(
        r"/PortailAS/appmanager/PortailAS/assure\?_nfpb=true&_pageLabel=as_conditions_generales_page.*", CguPage
    )
    subscription_page = URL(
        r"/PortailAS/appmanager/PortailAS/assure\?_nfpb=true&_pageLabel=as_info_perso_page", SubscriptionPage
    )
    ...
    def handle_otp(self):
        if not self.otp_form_data or not self.otp_form_url:
            self.logger.info(
                "We have an OTP but we don't have the OTP form and/or the OTP url." + " Restarting the login process..."
            )
