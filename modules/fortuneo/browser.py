...
class FortuneoBrowser(OAuth2PKCEMixin, TwoFactorBrowser):
    ...
    card_history = URL(
        r".*/prive/mes-comptes/compte-courant/carte-bancaire/encours-debit-differe\.jsp.*", CardHistoryPage
    )
    pea_history = URL(
    ...
    loan_contract = URL(
        r"/fr/prive/mes-comptes/credit-immo/contrat-credit-immo/contrat-pret-immobilier.jsp.*", LoanPage
    )
    ...
    confirm_recipient = URL(
        r"/fr/prive/mes-comptes/compte-courant/.*/confirmer-ajout-compte-externe.jsp", ConfirmRecipientPage
    )
    ...
    validate_transfer = URL(
        r"/fr/prive/mes-comptes/compte-courant/.*/verifier-saisie-virement.jsp", ValidateTransferPage
    )
    ...
    def handle_login_error(self, error):
        ...
        browserunavailable_regex = re.compile("Nous ne pouvons donner suite à votre demande" + "|Certificat invalide")
