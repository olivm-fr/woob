...
class LoginPage(PartialHTMLPage):
    ...
    def on_load(self):
        error_msg = CleanText('//div[contains(@class, "blocmsg err")] | //div[contains(@class, "blocmsg alerte")]')(
            self.doc
        )
        wrong_pass_regex = re.compile(
            "|".join(
                (
...
class AppValidationPage(Page):
    ...
    def get_polling_id(self):
        return Regexp(CleanText('//script[contains(text(), "transactionId")]'), r"transactionId: '(.{49})', get")(
            self.doc
        )
...
class MobileConfirmationPage(PartialHTMLPage, AppValidationPage):
    def is_here(self):
        ...
        if "code de confirmation vient de vous être envoyé par" in CleanText(
            '//div[contains(@id, "OTPDeliveryChannelText")]'
        )(self.doc) or CleanText('//*[contains(text(), "confirmer votre connexion avec Safetrans")]')(self.doc):
            return False
        return (
    ...
    def skip_redo_twofa(self):
        ...
        link = Attr(
            '//li[contains(text(), "confirmer votre identité plus tard")]//a[contains(@href, "Bypass") and contains(text(), "cliquez ici")]',
            "href",
            default=None,
        )(self.doc)
        if link:
    ...
    def check_bypass(self):
        link = Attr(
            '//a[contains(text(), "Accéder à mon Espace Client sans Confirmation Mobile") or contains(text(), "accéder à votre espace client")]',
            "href",
            default=None,
        )(self.doc)
        if link:
...
class SafeTransPage(PartialHTMLPage, AppValidationPage):
    ...
    def is_here(self):
        return "Authentification forte" in CleanText('//p[contains(@id, "title")]')(self.doc) and CleanText(
            '//*[contains(text(), "confirmer votre connexion avec Safetrans")]'
        )(self.doc)
...
class OtpValidationPage(PartialHTMLPage):
    def is_here(self):
        return "code de confirmation vient de vous être envoyé par" in CleanText(
            '//div[contains(@id, "OTPDeliveryChannelText")]'
        )(self.doc)
...
class EmptyPage(LoggedPage, HTMLPage):
    ...
    def on_load(self):
        ...
        action_needed = CleanText(
            '//p[contains(text(), "Votre Carte de Clés Personnelles") and contains(text(), "est révoquée")]'
        )(self.doc) or CleanText('//p[contains(text(), "Avant de passer toute opération sur ce site")]')(self.doc)
        if action_needed:
            raise ActionNeeded(locale="fr-FR", message=action_needed)
        maintenance = CleanText(
            '//td[@class="ALERTE"]/p/span[contains(text(), "Dans le cadre de l\'amélioration de nos services, nous vous informons que le service est interrompu")]'
        )(self.doc)
        if maintenance:
            raise BrowserUnavailable()
...
class UserSpacePage(LoggedPage, HTMLPage):
    def on_load(self):
        if self.doc.xpath('//form[@id="GoValider"]'):
            raise ActionNeeded(
                locale="fr-FR",
                message="Le site du contrat Banque à Distance a besoin d'informations supplémentaires",
                action_type=ActionType.FILL_KYC,
            )
        personal_infos = CleanText('//form[@class="_devb_act ___Form"]//div[contains(@class, "bloctxt")]/p[1]')(
            self.doc
        )
        if "Afin de compléter vos informations personnelles, renseignez le formulaire ci-dessous" in personal_infos:
            raise ActionNeeded(
                locale="fr-FR",
                message="Le site nécessite la saisie des informations personnelles de l'utilisateur.",
                action_type=ActionType.FILL_KYC,
            )
...
class item_account_generic(ItemElement):
    ...
    def loan_condition(self, check_no_details=False):
        ...
        if (
            ...
            if details and "cloturé" not in CleanText('//form[@id="P:F"]//div[@class="blocmsg info"]//p')(details.doc):
                ...
                if (
                    check_no_details
                ):  # check_no_details is used to determine if condition should check the absence of details, otherwise we still check the presence of details
    ...
    obj_label = Label(
        CleanText(
            './td[1]/a/text() | ./td[1]/a//span[@class and not(contains(@class, "doux"))] | ./td[1]/div/a[has-class("cb")]'
        )
    )
    ...
    def parse(self, el):
        ...
        for td in el.xpath("./td[2] | ./td[3]"):
            try:
                ...
                has_child_def_card = CleanText(
                    './/following-sibling::tr[1]//span[contains(text(), "Dépenses cartes prélevées")]'
                )(el)
        ...
        if _id in self.parent.objects:
            ...
            if not page.browser.is_new_website:
                ...
                date = parse_french_date(
                    Regexp(Field("label"), r"Fin (.+) (\d{4})", "01 \\1 \\2")(self)
                ...
                for elem in page.doc.xpath(card_xpath):
                    ...
                    next_month = Link('./following-sibling::tr[contains(@class, "encours")][1]/td[1]//a', default=None)(
                        self
                    )
                    if next_month:
    ...
    def is_revolving(self, label):
        return (
            any(
                revolving_loan_regex.search(label)
...
class AccountsPage(LoggedPage, HTMLPage):
    def has_no_account(self):
        return CleanText(
            '//td[contains(text(), "Votre contrat de banque à distance ne vous donne accès à aucun compte.")]'
        )(self.doc)
    ...
    class iter_accounts(ListElement):
        ...
        class item_account(item_account_generic):
            def condition(self):
                ...
                ) and item_account_generic.condition(self)
        ...
        class item_loan(item_account_generic):
            ...
            obj_total_amount = Async("details") & Coalesce(
                CleanDecimal.French(
                    CleanText('//tr[th[contains(text(), "Montant accord")]]/td', children=False), default=NotAvailable
                ),
                CleanDecimal.French(
                    '//th[span[contains(text(), "Montant accord")]]/following-sibling::td[1]', default=NotAvailable
                ),
            )
            obj_rate = Async("details") & Coalesce(
                CleanDecimal.French('//tr[th[contains(text(), "Taux")]]/td', default=NotAvailable),
                CleanDecimal.French(
                    '//th[span[contains(text(), "Taux")]]/following-sibling::td[1]', default=NotAvailable
                ),
            )
            ...
            obj_maturity_date = Async("details") & Date(
                Coalesce(
                    ...
                    CleanText(
                        '//th[span[contains(text(), "Date de fin")]]/following-sibling::td[1]', default=NotAvailable
                    ),
                ),
                dayfirst=True,
                default=NotAvailable,
            ...
            obj_next_payment_amount = Async("details") & Coalesce(
                CleanDecimal.French(  # in case of deferred loan
                    '//tr[th[contains(text(), "Prochain terme de franchise")]]/td', default=NotAvailable
                ),
                CleanDecimal.French(  # in case of deferred loan
                ...
                CleanDecimal.French(
                    '//th[span[contains(text(), "Prochaine échéance")]]/following-sibling::td[1]', default=NotAvailable
                ),
            )
            ...
            obj__insurance_url = Async("details") & Link(
                '//a[span[contains(text(), "Assurance emprunteur")]]', default=NotAvailable
            )
            ...
            def obj_deferred(self):
                ...
                if CleanText('//div/p[contains(text(), "actuellement en franchise de remboursement")]')(
        ...
        class item_revolving_loan(item_account_generic):
            ...
            obj_total_amount = Async("details") & CleanDecimal.French(
                '//tr[th[contains(text(), "votre disposition")]]/td[1]', default=NotAvailable
            )
            obj_available_amount = Async("details") & CleanDecimal.French(
                '//tr[th[contains(text(), "disponible")]]/td', default=NotAvailable
            )
...
class NewAccountsPage(NewHomePage, AccountsPage):
    def get_agency(self):
        return Regexp(
            CleanText('//script[contains(text(), "lien_caisse")]', default=""), r'(https://[^"]+)', default=""
        )(self.doc)
    ...
    class get_advisor(ItemElement):
        ...
        obj_name = Regexp(CleanText('//script[contains(text(), "Espace Conseiller")]'), r"consname.+?([\w\s]+)")
...
class Pagination:
    ...
    def next_month(self):
        ...
        try:
            form["moi"] = self.page.doc.xpath('//select[@id="moi"]/option[@selected]/following-sibling::option')[
                0
            ].attrib["value"]
            form["page"] = 1
        except IndexError:
        ...
        class item(ItemElement):
            ...
            def parse(self, el):
                ...
                for option in options:
                    ...
                    active_card = CleanText(
                        '%s[td[text()="Active"]][1]/td[2]' % xpath, replace=[(" ", "")], default=None
                    )(card_list_page.doc)
                ...
                xpath = '//table[has-class("liste")]/tbody/tr'
                active_card = CleanText('%s[td[text()="Active"]][1]/td[2]' % xpath, replace=[(" ", "")], default=None)(
                    page.doc
                )
                for cards in page.doc.xpath(xpath):
                    if CleanText(cards.xpath("./td[1]"))(self) != "Active":
                        self.page.browser.unavailablecards.append(
                            CleanText(cards.xpath("./td[2]"), replace=[(" ", "")])(self)
                        )
...
class Transaction(FrenchTransaction):
    PATTERNS = [
        ...
        (
            re.compile(r"REMISES (?P<dd>\d{2})(?P<mm>\d{2})(?P<yy>\d{2}) (?P<text>.*) CB \d+"),
            FrenchTransaction.TYPE_DEPOSIT,
        ),
        (
            re.compile(r"^(?P<text>.*)\s?(CARTE |PAYWEB)?\d+ PAIEMENT CB\s+(?P<dd>\d{2})(?P<mm>\d{2}) ?(.*)$"),
        ...
        (
            re.compile(r"^Regroupement \d+ PAIEMENTS (?P<dd>\d{2})(?P<mm>\d{2}) (?P<text>.*) CARTE \d+ ?(.*)$"),
            FrenchTransaction.TYPE_CARD,
        ),
        ...
        (
            re.compile(r"^(?P<dd>\d{2})/(?P<mm>\d{2})/(?P<yy>\d{4}) RETRAIT DAB (?P<text>.*)"),
            FrenchTransaction.TYPE_WITHDRAWAL,
        ),
...
class CardsActivityPage(LoggedPage, HTMLPage):
    def companies_link(self):
        ...
        for tr in self.doc.xpath(
            '//table[@summary="Liste des titulaires de contrats cartes" or @summary="Liste des tiers"]//tr'
        ):
    ...
    class iter_cards(TableElement):
        ...
        def next_page(self):
            ...
            if numtie is None:
                numtie = regex.filter(unquote(Attr('//a[contains(@class, "act popdetail")]', "href")(self)))
...
class OperationsPage(LoggedPage, HTMLPage):
        ...
        class item(Transaction.TransactionElement):
            ...
            class OwnRaw(Filter):
                def __call__(self, item):
                    ...
                    parts = (
                        re.sub(r"Détail|Date de valeur\s+:\s+\d{2}/\d{2}(/\d{4})?", "", txt.strip())
    ...
    def get_balance(self):
        return CleanDecimal.French('//span[contains(text(), "Dont opérations enregistrées")]', default=NotAvailable)(
            self.doc
        )
    ...
    def get_parent_id(self):
        ...
        return Regexp(
            Coalesce(
                CleanText('//tr[th[contains(text(), "R.I.B")]]/td', replace=[(" ", "")], default=NotAvailable),
                CleanText(
        ...
        class item(Transaction.TransactionElement):
            ...
            def obj_commission(self):
                ...
                if labels.search(raw) and "Intérêts" in raw:
                    ...
                    return sum([CleanDecimal.French(sign="-").filter(com) for com in commissions])
        ...
        class item(Transaction.TransactionElement):
            ...
            def parse(self, el):
                ...
                if not self.env["date"]:
                    try:
                        d = CleanText('//select[@id="moi"]/option[@selected]')(self) or re.search(
                            r"pour le mois de (.*)",
                            "".join(w.strip() for w in self.page.doc.xpath('//div[@class="a_blocongfond"]/text()')),
                        ).group(1)
                    except AttributeError:
                        d = Regexp(
                            CleanText('//p[has-class("restriction")]'),
...
class CardPage(OperationsPage, LoggedPage):
    ...
    class get_history(Pagination, ListElement):
        ...
        class list_history(Transaction.TransactionsElement):
            ...
            def parse(self, el):
                label = CleanText('//span[contains(text(), "Achats")]/following-sibling::span[2]')(el) or CleanText(
                    '//*[contains(text(), "Achats")]'
                )(el)
                if not label:
                    return
                try:
            ...
            class item(Transaction.TransactionElement):
                ...
                def parse(self, el):
                    ...
                    text_date = CleanText('//span[contains(text(), "Achats")]/following-sibling::span[2]')(
                        self
                    ...
                    self.env["original_amount"] = (
                        CleanDecimal(replace_dots=True).filter(original_amount)
                        if original_amount is not None
                        else NotAvailable
                    )
                    self.env["original_currency"] = (
...
class CardPage2(CardPage, HTMLPage, XMLPage):
    ...
    class get_history(ListElement):
        class list_history(Transaction.TransactionsElement):
            ...
            def condition(self):
                return not CleanText('//td[contains(., "Aucun mouvement")]', default=False)(self) or not CleanText(
                    '//td[contains(., "Aucune opération")]', default=False
                )(self)

            class item(Transaction.TransactionElement):
                def condition(self):
                    # Withdraw transactions are also presents on the checking account
                    return len(self.el.xpath("./td")) >= 4 and not CleanText(TableCell("commerce"))(self).startswith(
                        "RETRAIT CB"
                    )

                obj_raw = Transaction.Raw(
                    Format("%s %s", CleanText(TableCell("commerce")), CleanText(TableCell("ville")))
                )
    ...
    class get_tr_merged(ListElement):
        class list_history(Transaction.TransactionsElement):
            ...
            class item(Transaction.TransactionElement):
                def condition(self):
                    return len(self.el.xpath("./td")) >= 4 and not CleanText(TableCell("operation"))(self).startswith(
                        "RETRAIT CB"
                    )
...
class LIAccountsPage(LoggedPage, HTMLPage):
    def is_euro_fund(self):
        ...
        return not CleanText('//table[has-class("liste") and not (@summary="Avances")]/tbody/tr[count(td)>=7]')(
            self.doc
        )
    ...
    class iter_li_accounts(TableElement):
        ...
        class item(ItemElement):
            ...
            obj_id = obj_number = Coalesce(
                Base(TableCell("label"), Regexp(CleanText(".//a"), r"Contrat ([^\s]*)", default=NotAvailable)),
    ...
    class iter_investment(TableElement):
        ...
        class item(ItemElement):
            ...
            obj_unitprice = CleanDecimal(
                TableCell("unitprice", default=NotAvailable), default=NotAvailable, replace_dots=True
            )
            ...
            def obj_srri(self):
                ...
                if TableCell("srri", default=None)(self) is not None:
                    srri = Base(
                        TableCell("srri"),
                        CleanDecimal('.//li[@class="ei_grad_scale_item_selected"]', default=NotAvailable),
                    )(self)
                    if srri:
                        return int(srri)
                return NotAvailable
            ...
            def obj_portfolio_share(self):
                portfolio_share_percent = CleanDecimal.French(TableCell("portfolio_share", default=None), default=None)(
                    self
                )
...
class PorPage(LoggedPage, HTMLPage):
    def get_action_needed_message(self):
        if self.doc.xpath('//form[contains(@action, "MsgCommerciaux")]') and self.doc.xpath(
            '//input[contains(@id, "Valider")]'
        ):
    ...
    class iter_por_accounts(TableElement):
        ...
        class item(ItemElement):
            ...
            def condition(self):
                ...
                is_liquidity = "LIQUIDITE" in CleanText(TableCell("raw_label"))(
                    self
                ...
                return not is_total and not is_liquidity and not is_global_view and not has_empty_balance
            ...
            def obj_valuation_diff_ratio(self):
                valuation_diff_ratio_percent = CleanDecimal.French(
                    TableCell("valuation_diff_ratio"), default=NotAvailable
                )(self)
    ...
    def fill(self, acc):
        ...
        if balance == "NS" and not acc.balance:
            ...
            if acc.balance:
                ...
                acc.balance = balance
        acc.valuation_diff = CleanDecimal(
            ele.xpath('.//td[contains(@id, "Variation")]'), default=Decimal(0), replace_dots=True
        )(ele)
        if balance:
        ...
        class item(ItemElement):
            ...
            def obj_quantity(self):
                ...
                if "+" in quantity:
                    ...
                    return CleanDecimal.French().filter(quantity_list[0]) + CleanDecimal.French().filter(
                        quantity_list[1]
                    )
                else:
            ...
            def obj_vdate(self):
                ...
                return Date(
                    Regexp(Attr("./img", "title", default=""), r"Cours au : (\d{2}/\d{2}/\d{4})\b", default=None),
                    dayfirst=True,
                    default=NotAvailable,
                )(td)
...
class PorInvestmentsPage(LoggedPage, HTMLPage):
    ...
    class iter_investment(TableElement):
        ...
        class item(ItemElement):
            ...
            def obj_portfolio_share(self):
                portfolio_share_percent = Base(TableCell("valuation"), CleanDecimal.French("./div[2]", default=None))(
                    self
                )
...
class PorHistoryDetailsPage(LoggedPage, HTMLPage):
    ...
    class fill_transaction(ItemElement):
        ...
        def obj_investments(self):
            ...
            investment.code = IsinCode(
                Regexp(CleanText('//td[@id="esdtdLibelleValeur"]'), r"\((.*)\)"), default=NotAvailable
            )(self)
            investment.code_type = IsinType(
                Regexp(CleanText('//td[@id="esdtdLibelleValeur"]'), r"\((.*)\)"), default=NotAvailable
            )(self)
            return [investment]
...
class PorMarketOrderDetailsPage(LoggedPage, HTMLPage):
    ...
    class fill_market_order(ItemElement):
        obj_stock_market = Regexp(
            CleanText('//td[contains(@id, "esdtdAchat")]/text()[contains(., "Sur")]'), r"Sur (.*)", default=NotAvailable
        )
...
class TransferPageCommon(LoggedPage, HTMLPage, AppValidationPage):
    ...
    def needs_personal_key_card_validation(self):
        return bool(
            CleanText(
                '//div[contains(@class, "alerte")]/p[contains(text(), "Cette opération nécessite une sécurité supplémentaire")]'
            )(self.doc)
        )
    ...
    def can_transfer(self, origin_account):
        if self.doc.xpath('//ul[@id="idDetailsListCptDebiterVertical:ul"]') or self.doc.xpath(
            '//ul[@id="idDetailListCptDebiter:ul"]'
        ):
    ...
    def check_data_consistency(self, account_id, recipient_id, amount, reason):
        assert account_id in CleanText('//div[div[p[contains(text(), "Compte à débiter")]]]', replace=[(" ", "")])(
            self.doc
        )
        assert recipient_id in CleanText(
            '//div[div[p[contains(text(), "%s")]]]' % self.SUMMARY_RECIPIENT_TITLE, replace=[(" ", "")]
        )(self.doc)

        exec_date = Date(
            Regexp(CleanText('//table[@summary]/tbody/tr[th[contains(text(), "Date")]]/td'), r"(\d{2}/\d{2}/\d{4})"),
            dayfirst=True,
        )(self.doc)
        r_amount = CleanDecimal('//table[@summary]/tbody/tr[th[contains(text(), "Montant")]]/td', replace_dots=True)(
            self.doc
        )
        assert r_amount == Decimal(amount)
        currency = FrenchTransaction.Currency('//table[@summary]/tbody/tr[th[contains(text(), "Montant")]]/td')(
            self.doc
        )

        if reason is not None:
            creditor_label = CleanText(".").filter(reason.upper()[:22])
            debitor_label = CleanText(
                '//table[@summary]/tbody/tr[th[contains(text(), "Intitulé pour le compte à débiter")]]/td'
            )(self.doc)
    ...
    class iter_recipients(ListElement):
        ...
        class item(MyRecipient):
            ...
            def obj_iban(self):
                l = [
                    a
                    for a in self.page.browser.get_accounts_list()
...
class VerifCodePage(LoggedPage, HTMLPage):
    HASHES = {
        (
            "b1b472cb6e6adc28bfdcc4bc86661fa7",
            "f8d9330f322575cb3d5853c347c4ed16",
    ...
    def get_error(self):
        errors = (
            ...
            CleanText(
                '//p[contains(text(), "Vous n\'êtes pas inscrit") and a[text()="service d\'identification renforcée"]]'
            )(self.doc),
...
class RecipientsListPage(LoggedPage, HTMLPage):
    ...
    def ask_auth_validation(self, recipient):
        ...
        if app_validation:
            self.browser.recipient_form["transactionId"] = Regexp(
                CleanText('//script[contains(text(), "transactionId")]'), r"transactionId: '(.{49})', get"
            )(self.doc)
            raise AppValidation(resource=recipient, message=app_validation)

        sms_validation = CleanText('//span[contains(text(), "Pour confirmer votre opération, indiquez votre ")]')(
            self.doc
        )
        if sms_validation:
    ...
    class iter_documents(TableElement):
        ...
        class item(ItemElement):
            ...
            obj_id = Format(
                "%s_%s_%s",
                Env("sub_id"),
                CleanText(TableCell("date"), replace=[("/", "")]),
    ...
    class iter_accounts(ListElement):
        ...
        class item(ItemElement):
            ...
            def condition(self):
                ...
                return "Dépenses" in CleanText('.//tr[1]/th/a[contains(@id,"C:more-card")]')(self) and (
                    CleanText(".//div[1]/p")(self) == "Active" or Field("coming")(self) != 0
                )
            ...
            def parse(self, el):
                ...
                if function_button:
                    ...
                    card_type_line = doc.xpath('//tbody/tr[th[contains(text(), "Débit des paiements")]]') or doc.xpath(
                        '//div[div/div/p[contains(text(), "Débit des paiements")]]'
                    )
                    if card_type_line:
                    ...
                    form = self.page.get_form(
                        id="C:P:F", submit=".//input[@name='%s']" % Field("_submit_button_name")(self)
                    )
        ...
        def next_page(self):
            ...
            if previous_month is not None:
                ...
                if not month_name:
                    ...
                    previous_month = next_page_same_month.doc.xpath(
                        "//div[has-class('ei_timeline')]//li[has-class('sep')]"
                    )[-1]
        ...
        class item(ItemElement):
            ...
            def parse(self, el):
                if CleanText('//a[contains(text(), "prélevés fin")]', default=None)(self):
                    ...
                    month_name = Regexp(
                        CleanText('//a[contains(text(), "prélevés fin")]'), r".* \(prélevés fin (\w+)\).*", default=""
                    )(self)
            ...
            obj_label = MultiJoin(CleanText(TableCell("commerce")), CleanText(TableCell("ville")), pattern=" ")
...
class InformationConfirmationPage(LoggedPage, HTMLPage):
    def get_confirmation_link(self):
        if HasElement('//h1[@class="titlecontent"]/text()="Confirmez votre numéro de téléphone portable"')(self.doc):
