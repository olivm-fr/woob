    ...
    def check_messages(self):
        ...
        try:
            for user in self._browser.get_users_list():
                if user["is_match"] is False or (
                    user["connection_status_indicator"] == 0 and user["unread_messages_count"] == 0
                ):
                    continue
                ...
                prompt = self.build_prompt(
                    self._browser.get_my_user(), self._browser.get_user(user["user_id"]), messages
                )
...
class BumbleModule(Module, CapDating, CapMessages, CapMessagesPost):
    ...
    def init_optimizations(self):
        ...
        self.add_optimization(
            "PROFILE_WALKER", ProfilesWalker(self.woob.scheduler, self.storage, self.browser, self.config["city"].get())
        )
        if self.config["chatgpt_key"].get():
            self.add_optimization(
                "AUTO_RESPONDER", AutoResponder(self.woob.scheduler, self.browser, self.config["chatgpt_key"].get())
            )
