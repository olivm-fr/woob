...
class SocieteGeneraleTwoFactorBrowser(TwoFactorBrowser):
    ...
    skippable_action_needed_page = URL(
        r"/icd/gax/data/users/administration/out-of-remedy-security-security-zone.json", SkippableActionNeededPage
    )
    ...
    def check_auth_method(self):
        ...
        if not auth_method:
            ...
            raise ActionNeeded(
                locale="fr-FR",
                message="Veuillez ajouter un numéro de téléphone sur votre banque et/ou activer votre Pass Sécurité.",
        ...
        if auth_method["unavailability_reason"] == "ts_non_enrole":
            raise ActionNeeded(
                locale="fr-FR",
                message="Veuillez ajouter un numéro de téléphone sur votre banque.",
                action_type=ActionType.ENABLE_MFA,
            )
        ...
        if auth_method["type_proc"].lower() == "auth_oob":
            ...
            self.location(
                "/sec/oob_sendooba.json", method="POST", headers={"Content-Type": "application/x-www-form-urlencoded"}
            )
            ...
            if auth_method["mode"] == "SMS":
                # SMS is sent here
                self.location("/sec/csa/send.json", data={"csa_op": "auth"})
    ...
    def handle_sms(self):
        if len(self.code) != 6:
            raise BrowserIncorrectPassword("Le Code Sécurité doit avoir une taille de 6 caractères")
...
class SocieteGenerale(SocieteGeneraleTwoFactorBrowser):
    ...
    # Recipient
    add_recipient = URL(r"/personnalisation/per_cptBen_ajouterFrBic.html", r"/lgn/url.html", AddRecipientPage)
    json_recipient = URL(
    ...
    life_insurance = URL(r"/asv/asvcns10.html", r"/asv/AVI/asvcns10a.html", r"/brs/fisc/fisca10a.html", LifeInsurance)
    ...
    life_insurance_invest_api = URL(
        r"/icd/avd/data/api/v1/detail-contrat-assurance-vie-authsec.json", LifeInsuranceInvestAPI
    )
    life_insurance_invest_api_2 = URL(
        r"/icd/avd/data/api/v1/contrat-assurance-vie-authsec.json", LifeInsuranceInvestAPI2
    )
    ...
    error = URL(r"https://static.sg.fr/pri/erreur.html", r"https://.*/pri/erreur.html", ErrorPage)
    login = URL(
    ...
    def iter_market_orders(self, account):
        ...
        for trial in range(2):
            ...
            if not self.market_orders.is_here():
                self.logger.warning(
                    "Landed on unknown page when trying to fetch market orders for account %s", account.label
                )
                return
            ...
            if self.page.orders_unavailable():
                if trial == 0:
                    self.logger.warning(
                        "Market Orders page is unavailable for account %s, retrying now.", account.label
                    )
                    continue
    ...
    def send_sms_to_user(self, recipient):
        ...
        raise AddRecipientStep(
            recipient, Value("code", label="Cette opération doit être validée par un Code Sécurité.")
        )
    ...
    def send_notif_to_user(self, recipient):
        ...
        raise AddRecipientStep(
            recipient, ValueBool("pass", label="Valider cette opération sur votre applicaton société générale")
        )
