...
def eval_decimal_amount(value, decimal_position):
    return Eval(lambda x, y: x / 10**y, CleanDecimal(Dict(value)), CleanDecimal(Dict(decimal_position)))
...
class JsonBasePage(LoggedPage, JsonPage):
    ...
    def on_load(self):
        if Dict("commun/statut")(self.doc).upper() == "NOK":
            ...
            if "le service est momentanement indisponible" in reason and Dict("commun/origine")(self.doc) != "cbo":
    ...
    class iter_accounts(DictElement):
        ...
        class item(ItemElement):
            ...
            def obj__is_json_histo(self):
                ...
                if Field("type")(self) == Account.TYPE_REVOLVING_CREDIT and not Dict("produit")(self) in (
                    "COMPTE_ALTERNA",
                    "AVANCE_PATRIMOINE",
                ):
                    return True
                # PLAN_EPARGNE_POPULAIRE account type history is not in json yet
                if Field("type")(self) == Account.TYPE_SAVINGS and not Dict("produit")(self) in (
                    "PLAN_EPARGNE_POPULAIRE",
                ):
                    return True
        ...
        for loan_type in loan_types_fields:
            for acc in Dict(f"donnees/{loan_type}")(self.doc):
                if CleanText(Dict("detailPret/idPrestation"))(acc) == loan._prestation_id:
                    ...
                    loan.next_payment_date = Date(
                        CleanText(Dict("detailPret/dateProchaineEcheance"), default=NotAvailable), default=NotAvailable
                    )(acc)
                    loan.used_amount = CleanDecimal.French(Dict("montantUtilise", default=None), default=NotAvailable)(
                        acc
                    )
                    loan.subscription_date = Date(
                        CleanText(Dict("detailPret/dateOuverture"), default=""), default=NotAvailable
                    )(acc)
                    loan.available_amount = CleanDecimal.French(
                        Dict("soldeReserveOuCapital", default=None), default=NotAvailable
                    )(acc)
...
class TransactionItemElement(ItemElement):
    ...
    def parse(self, el: ItemElement) -> None:
        ...
        if motif_raw:
            self.env.update(
                dict(zip_longest(("motive", "ref", "sepa_mandate"), re.split("(?: (?:REF:|MANDAT) )", motif_raw)))
            )
            ...
            loan_data.update(
                {
                    (
                        key,
                        CleanDecimal.French().filter(m[key]) if m[key] else NotAvailable,
                    )
                    for key in (
                        "principal_amount",
                        "interest_amount",
...
class HistoryPage(JsonBasePage):
    ...
    def hist_pagination(self, condition):
        all_conditions = {
            ...
            "intraday": (
                not Dict("donnees/listeOperations", default=None)(self.doc),
                Dict("donnees/listeOperations", default=None)(self.doc)
                and not Dict("donnees/listeOperations/0/statutOperation", default=None)(self.doc) == "INTRADAY",
                not Dict("donnees/recapitulatifCompte/chargerPlusOperations", default=None)(self.doc),
    ...
    class iter_card_transactions(DictElement):
        ...
        class item(TransactionItemElement):
            def condition(self):
                ...
                conditions = (
                    Dict("idOpe")(self)
                    and Regexp(CleanText(Dict("idOpe")), r"(\w+)/", default=NotAvailable)(self) in ["0", "null"],
    ...
    class iter_history(TableElement):
        ...
        def condition(self):
            no_transaction_msg = any(
                (
                    self.xpath(
                        '//div[contains(text(), "Aucune opération trouvée sur la période de restitution possible")]'
                    ),
                    self.xpath(
                        '//div[contains(text(), "Aucune opération n\'a été réalisée depuis le dernier relevé")]'
                    ),
                )
            )
...
class LifeInsuranceInvestDetailsAPI(LoggedPage, JsonPage):
    ...
    class fill_life_insurance_investment(ItemElement):
        def parse(self, el):
            ...
            diff_ratio = CleanDecimal.SI(Dict("pourcentagePerformance", default=None), default=None)(current_fund)
...
class LifeInsuranceHistory(LifeInsurance):
    ...
    class iter_history(TableElement):
        ...
        class item(ItemElement):
            def condition(self):
                return CleanText(TableCell("_status"))(self) == "Réalisé" and MyDecimal(
                    TableCell("amount"), default=NotAvailable
                )(self)
...
class MarketPage(HTMLLoggedPage):
    ...
    class iter_investments(TableElement):
        ...
        def condition(self):
            return not "PAS DE VALEURS DETENUES ACTUELLEMENT SUR CE COMPTE" in CleanText('//td[@class="MessErreur"]')(
                self.el
            )
...
class MarketOrderDetailPage(LoggedPage, HTMLPage):
    ...
    class fill_market_order(ItemElement):
        ...
        obj_quantity = CleanDecimal.French(
            '//td[contains(text(), "Quantité demandée")]//following-sibling::td[1]', default=NotAvailable
        )
        obj_ordervalue = CleanDecimal.French(
            '//td[contains(text(), "Cours limite")]//following-sibling::td[1]', default=NotAvailable
        )
        obj_amount = CleanDecimal.French(
            '//td[contains(text(), "Montant")]//following-sibling::td[1]', default=NotAvailable
        )
        obj_unitprice = CleanDecimal.French(
            '//td[contains(text(), "Cours d\'exécution")]//following-sibling::td[1]', default=NotAvailable
        )
...
class AdvisorPage(LoggedPage, XMLPage):
    ...
    def get_advisor(self):
        ...
        advisor.address = Format("%s %s %s", CleanText("//ruadre"), CleanText("//cdpost"), CleanText("//loadre"))(
            self.doc
        )
...
class HTMLProfilePage(HTMLLoggedPage):
    def on_load(self):
        msg = CleanText('//div[@id="connecteur_partenaire"]', default="")(self.doc) or CleanText("//body", default="")(
            self.doc
        )
        service_unavailable_msg = CleanText('//div[contains(@class, "error")]//span[contains(text(), "indisponible")]')(
            self.doc
        )
    ...
    def get_profile(self):
        profile = Person()
        profile.name = Regexp(
            CleanText('//div[@id="dcr-conteneur"]//div[contains(text(), "PROFIL DE")]'), r"PROFIL DE (.*)"
        )(self.doc)
        profile.address = CleanText(
            '//div[@id="dcr-conteneur"]//div[contains(text(), "ADRESSE")]/following::table//tr[3]/td[2]'
        )(self.doc)
        profile.address += " " + CleanText(
            '//div[@id="dcr-conteneur"]//div[contains(text(), "ADRESSE")]/following::table//tr[5]/td[2]'
        )(self.doc)
        profile.address += " " + CleanText(
            '//div[@id="dcr-conteneur"]//div[contains(text(), "ADRESSE")]/following::table//tr[6]/td[2]'
        )(self.doc)
        profile.country = CleanText(
            '//div[@id="dcr-conteneur"]//div[contains(text(), "ADRESSE")]/following::table//tr[7]/td[2]'
        )(self.doc)
